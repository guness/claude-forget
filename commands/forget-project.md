...
- This is **far** more destructive than `/forget`: it removes **every** Claude-realm reference to this project — *all* sessions and memory, prompt history, per-session state, the `~/.claude.json` project entry (trust, allowedTools, MCP), security audit-log lines, and config backups.
...
- **To back out, run `/forget-project cancel`.**
