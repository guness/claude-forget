...
- Nothing has been deleted yet — `/forget:session` only **armed** the wipe. (A slash command cannot reset the context window or safely delete the live transcript on its own; this is a verified Claude Code limitation, so the wipe is finished at the moment the session ends.)
